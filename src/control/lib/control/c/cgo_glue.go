...
// requireRank rejects the CRT_NO_RANK sentinel (== ranklist.NilRank) where a
// specific rank is required.
func requireRank(rank C.d_rank_t) error {
	if ranklist.Rank(rank) == ranklist.NilRank {
		return fmt.Errorf("a specific rank is required, got CRT_NO_RANK: %w",
			daos.InvalidInput)
	}
	return nil
}
...
func errorToRC(err error) int {
	...
	// MS conditions arrive wire-stringified (their types don't survive
	// gRPC); match the canonical sentinels' renderings, not local prose.
	if msg := err.Error(); strings.Contains(msg, (&system.ErrPoolNotFound{}).Error()) {
		return int(daos.Nonexistent)
	} else if strings.Contains(msg, checker.FaultCheckerEnabled.Error()) {
		return int(daos.Busy)
	}
...
// pool-create request. Entries handled outside the property registry
// (ACL, owner, owner_group, svc_list) are silently skipped; all other
// unrecognized property types return daos.NotSupported.
func propsFromC(cProps *C.daos_prop_t) ([]*daos.PoolProperty, error) {
	...
	for i := range entries {
		...
		// These entries are handled by poolCreateExtrasFromC, not the
		// registry. svc_list is read-only and unsettable at create time.
		switch propNum {
...
// poolCreateExtrasFromC extracts ACL entries and owner/owner-group overrides
// from a pool-create prop list and applies them to req. ACL entries come from
// DAOS_PROP_PO_ACL (dpe_val_ptr → struct daos_acl *); owner/owner_group come
// from DAOS_PROP_PO_OWNER/DAOS_PROP_PO_OWNER_GROUP (dpe_str values such as
// "alice@" or "admins@").
//
// The old dmg_pool_create helper (065f68a3c0~1:src/common/tests_dmg_helpers.c)
// passed ACL entries via a tempfile to --acl-file, but did NOT read
// DAOS_PROP_PO_OWNER or DAOS_PROP_PO_OWNER_GROUP from the prop at all — owner
// was derived solely from uid/gid.  The new path honors both for full parity.
func poolCreateExtrasFromC(cProps *C.daos_prop_t, req *control.PoolCreateReq) error {
	if cProps == nil || cProps.dpp_nr == 0 {
		return nil
	}
	...
	for i := range entries {
		...
		switch propNum {
		case C.DAOS_PROP_PO_ACL:
			acl := (*C.struct_daos_acl)(C.get_dpe_val_ptr(entry))
			if acl == nil {
				continue
			}
			var aceStrs **C.char
			var aceNr C.size_t
			if rc := C.daos_acl_to_strs(acl, &aceStrs, &aceNr); rc != 0 {
				return fmt.Errorf("daos_acl_to_strs: %w", daos.Status(rc))
			}
			nr := int(aceNr)
			goStrs := make([]string, nr)
			if nr > 0 && aceStrs != nil {
				cSlice := unsafe.Slice(aceStrs, nr)
				for j := 0; j < nr; j++ {
					goStrs[j] = C.GoString(cSlice[j])
					C.free(unsafe.Pointer(cSlice[j]))
				}
			}
			if aceStrs != nil {
				C.free(unsafe.Pointer(aceStrs))
			}
			if req.ACL == nil {
				req.ACL = &control.AccessControlList{}
			}
			req.ACL.Entries = goStrs

		case C.DAOS_PROP_PO_OWNER:
			if s := C.get_dpe_str(entry); s != nil {
				req.User = C.GoString(s)
			}

		case C.DAOS_PROP_PO_OWNER_GROUP:
			if s := C.get_dpe_str(entry); s != nil {
				req.UserGroup = C.GoString(s)
			}
		}
	}
	return nil
