...
func TestControlC_StatusFromMsg(t *testing.T) {
...
func TestControlC_UidGidLookupErrorsAreInvalidInput(t *testing.T) {
...
func TestControlC_ErrorToRC(t *testing.T) {
	for name, tc := range map[string]struct {
		err   error
		expRC int
	}{
		...
		"pool service not found, no embedded code": {
			err:   fmt.Errorf("pool destroy failed: %s", system.ErrPoolUUIDNotFound(uuid.New())),
			expRC: int(daos.Nonexistent),
		},
		"checker enabled blocks normal ops": {
			err:   fmt.Errorf("pool create failed: %s", checker.FaultCheckerEnabled),
			expRC: int(daos.Busy),
		},
		"daos.Status - NoSpace": {
			err:   daos.NoSpace,
...
func TestControlC_SummarizeRPCErr(t *testing.T) {
