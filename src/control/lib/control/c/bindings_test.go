...
func TestControlC_BogusHandlePanicRecovery(t *testing.T) {
	...
	// cause cgo.Handle.Value() to panic; recoverExport maps that to MiscError.
	const bogus = cgo.Handle(0xdeadbeef)

	if rc := callCheckSwitch(bogus, true); rc != int(daos.MiscError) {
		t.Fatalf("rc=%d, want %d (MiscError)", rc, int(daos.MiscError))
	}
}
