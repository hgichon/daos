...
generate_oids(struct test_ctx *ctx)
{
	...
	for (i = 0; i < ctx->num_oids; i++) {
		ctx->oids[i].oid.hi = 0;
		ctx->oids[i].oid.lo = ((uint64_t)rand() << 32) | i;
		daos_obj_set_oid_by_class(&ctx->oids[i].oid, 0, ctx->oclass, 0);
	}
...
int
capture_layouts(struct test_ctx *ctx, struct oid_layout *layouts, double *ms_place_out)
{
	...
	*ms_place_out = 0.0;
	for (i = 0; i < ctx->num_oids; i++) {
		...
		md.omd_id  = ctx->oids[i].oid;
		md.omd_ver = pool_map_get_version(ctx->pool_map);
		md.omd_pda = 1;
		layout     = NULL;
		...
		*ms_place_out +=
		...
		if (rc != 0 || layout == NULL) {
			...
			layouts[i].oid = ctx->oids[i].oid;
			layouts[i].nr  = 0;
			continue;
		}

		layouts[i].oid      = ctx->oids[i].oid;
		layouts[i].nr       = layout->ol_nr;
		layouts[i].grp_size = layout->ol_grp_size;
		layouts[i].grp_nr   = layout->ol_grp_nr;

		/*
		 * (Re)allocate shard buffer if not yet allocated or if the
		 * layout size changed (e.g., grown by pl_map_extend during
		 * an add/extend operation).
		 */
		if (layouts[i].ol_shards != NULL && layouts[i].nr != layout->ol_nr)
			D_FREE(layouts[i].ol_shards);

		if (layouts[i].ol_shards == NULL) {
			D_ALLOC_ARRAY(layouts[i].ol_shards, layout->ol_nr);
		...
		memcpy(layouts[i].ol_shards, layout->ol_shards,
		       layout->ol_nr * sizeof(struct pl_obj_shard));
		pl_obj_layout_free(layout);
	}
	return 0;
}
...
void
free_layouts(struct oid_layout *layouts, int num_oids)
{
	int i;
	if (layouts == NULL)
		return;
 ...
 *   Compare orig_layout against post_layout directly.  The number of shards
 ...
 *   copy of orig_layout.  The resulting merged layout is then compared
 ...
 * @param orig_layout Layout captured before the operation (not modified)
 ...
 */
int
compare_layout(struct test_ctx *ctx, struct oid_layout *orig_layout, struct shard_diff *diff,
	       struct oid_layout *post_layout, int max_diff)
{
	int i;
	int j;
	int k;
	 ...
	 * ------------------------------------------------------------------ */
	for (i = 0; i < ctx->num_oids; i++) {
		for (j = 0; j < (int)orig_layout[i].grp_nr; j++) {
			int shard_diff_cnt = 0;

			for (k = 0; k < (int)orig_layout[i].grp_size; k++) {
				struct pl_obj_shard *s1;
				struct pl_obj_shard *s2;

				s1 = &orig_layout[i].ol_shards[j * orig_layout[i].grp_size + k];
				s2 = &post_layout[i].ol_shards[j * post_layout[i].grp_size + k];
			...
			if (shard_diff_cnt > max_diff) {
				fprintf(stderr,
					...
					"(max allowed: %d)\n",
					i, j, shard_diff_cnt, orig_layout[i].grp_size, max_diff);
				return -DER_IO;
			}
		}
	}
	...
	for (i = 0; i < ctx->num_oids; i++) {
		...
		int                  nr = orig_layout[i].nr;

		/* Temporary copy of orig so we do not mutate the original. */
		D_ALLOC_ARRAY(merged, nr);
		if (merged == NULL) {
			fprintf(stderr, "  ERROR: failed to allocate merged layout\n");
			return -DER_NOMEM;
		}
		memcpy(merged, orig_layout[i].ol_shards, nr * sizeof(struct pl_obj_shard));
		...
		for (j = 0; j < diff[i].nr; j++) {
			...
			if (shard_idx < (uint32_t)nr)
				merged[shard_idx].po_target = diff[i].spare_tgts[j];
		}

		/* Merged layout must match post_layout with zero mismatches. */
		for (j = 0; j < (int)orig_layout[i].grp_nr; j++) {
			for (k = 0; k < (int)orig_layout[i].grp_size; k++) {
				int                  idx = j * orig_layout[i].grp_size + k;
...
int
validate_configuration(int64_t nodes, int64_t ranks, int64_t targets, int64_t obj_count,
		       const char *object_class_str, struct operation *operations,
		       int operation_count)
{
	...
	if (nodes <= 0 || ranks <= 0 || targets <= 0 || obj_count <= 0) {
		fprintf(
		    stderr,
	...
	total_ranks   = nodes * ranks;
	total_targets = total_ranks * targets;
	...
	for (i = 0; i < operation_count; i++) {
		...
		if (op->type == OP_ADD) {
			...
			nodes += op->nr_args;
			total_ranks += (int64_t)op->nr_args * ranks;
			total_targets += (int64_t)op->nr_args * ranks * targets;
			continue;
		}
			     ...
			     double *ms_rebuild_out)
{
	...
	pool_map_init_in_fseq(ctx->pool_map);
	 ...
	 * spare targets at the intermediate state.
	 */
	*ms_rebuild_out = 0.0;
	for (i = 0; i < ctx->num_oids; i++) {
		md.omd_id  = ctx->oids[i].oid;
		...
		diff[i].nr = pl_obj_find_rebuild(ctx->pl_map, PLT_LAYOUT_VERSION, &md, NULL, ver,
						 diff[i].spare_tgts, diff[i].shard_ids, 128);
